...
@RestController
@RequestMapping("/entidadMedica")
public class EntidadMedicaController {

    @Autowired
    private UnidadMedicaServiceImpl entidadMedicaService;
    ...
    public ResponseEntity<List<UnidadMedica>> getEntidadesByMunicipio(@RequestBody BusquedaDTO busqueda) {
        List<UnidadMedica> entidades = entidadMedicaService.buscarPorMunicipio(busqueda.getMunicipio());
    ...
    public ResponseEntity<List<String>> getDistinctMunicipios() {
        List<String> municipios = entidadMedicaService.getDistinctMunicipios();
    ...
    public ResponseEntity<List<UnidadMedica>> getEntidadesByMunicipios(@RequestBody List<String> municipios) {
        ...
        for (String municipio : municipios) {
            entidades.addAll(entidadMedicaService.buscarPorMunicipio(municipio));
        }
        if (entidades.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<List<UnidadMedica>> getAllEntidades() {
        List<UnidadMedica> entidades = entidadMedicaService.getAllEntidadesMedicas();
        if (entidades.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(entidades);
    ...
    public ResponseEntity<UnidadMedica> addEntidadMedica(@RequestBody UnidadMedica entidadMedica) {
        UnidadMedica nuevaEntidad = entidadMedicaService.addUnidadMedica(entidadMedica);
    ...
    public ResponseEntity<Void> deleteEntidadMedica(@PathVariable Integer id) {
        entidadMedicaService.deleteUnidadMedica(id);
    ...
    public ResponseEntity<UnidadMedica> updateEntidadMedica(@PathVariable Integer id, @RequestBody UnidadMedica entidadMedica) {
        UnidadMedica entidadActualizada = entidadMedicaService.updateUnidadMedica(id, entidadMedica);
